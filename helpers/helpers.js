...
async function getPoolAddress(_factory, _token0, _token1, _fee, _exchangeName) {
  let poolAddress
  if (_exchangeName === "Camelot V3") {
    // Camelot V3 uses Algebra protocol with poolByPair function (no fee parameter)
    poolAddress = await _factory.poolByPair(_token0, _token1)
  } else {
    // Uniswap V3 uses getPool with fee parameter
    poolAddress = await _factory.getPool(_token0, _token1, _fee)
  }
  return poolAddress
}

async function getPoolContract(_exchange, _token0, _token1, _fee, _provider) {
  const poolAddress = await getPoolAddress(_exchange.factory, _token0, _token1, _fee, _exchange.name)
...
async function calculatePrice(_pool, _token0, _token1, _exchangeName) {
  // Understanding Uniswap V3 prices
  // --> https://blog.uniswap.org/uniswap-v3-math-primer

  let sqrtPriceX96
  if (_exchangeName === "Camelot V3") {
    // Camelot V3 (Algebra) - try globalState() first, fallback to slot0()
    try {
      const [price] = await _pool.globalState()
      sqrtPriceX96 = price
    } catch (error) {
      console.log("  ⚠️ globalState() failed, trying slot0()...")
      const [price] = await _pool.slot0()
      sqrtPriceX96 = price
    }
  } else {
    // Uniswap V3 uses slot0()
    const [price] = await _pool.slot0()
    sqrtPriceX96 = price
  }
